...
FROM store
UNION ALL 
SELECT DISTINCT order_id, item_2_id as item_id
FROM store
WHERE item_2_id IS NOT NULL
UNION ALL 
SElECT DISTINCT order_id, item_3_id as item_id
FROM store
WHERE item_3_id IS NOT NULL;
...
WHERE customers.customer_id = orders.customer_id AND orders.order_date > '2019-07-25';
...
SELECT item_id, COUNT(*) as number_of_orders
FROM all_items
GROUP BY item_id;
